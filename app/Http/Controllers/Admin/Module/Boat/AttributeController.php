...

{
    public function index(){
        $datas = Attribute::where('is_active', 1)->where('service', 'boat')->get()->reverse();
        $sl = 0;

        return view('admin.module.boat.attribute.index', compact('datas', 'sl'));
    }

    public function store(Request $request){
        $validatedData = $request->validate([
            'name' => ['required', 'unique:attributes,name,'.$request->id.',id,service,boat'],
        ...
        return back()->with('success', 'New Boat Attribute Added Successfully!');
    }
    
    public function edit($id){
        $data = Attribute::findorFail($id);

        return view('admin.module.boat.attribute.details', compact('data'));
    }
    
    public function update(Request $request, $id){
        $validatedData = $request->validate([
            'name' => ['required', 'unique:attributes,name,'.$id.',id,service,boat'],
            'position' => ['nullable', 'numeric'],
        ]);
        $parentName = 'boat';
        $data = Attribute::findorFail($id);
        ...
        return back()->with('success', 'Boat Attribute Updated Successfully!');
    }
    ...
    public function termList($id){
        ...
        return view('admin.module.boat.attribute.termList', compact('attributeInfo', 'datas', 'sl'));
    }
    ...
    public function termEdit($id){
        ...
        return view('admin.module.boat.attribute.termDetails', compact('data'));
    }
