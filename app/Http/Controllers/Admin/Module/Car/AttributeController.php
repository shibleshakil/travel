...

{
    public function index(){
        $datas = Attribute::where('is_active', 1)->where('service', 'car')->get()->reverse();
        $sl = 0;

        return view('admin.module.car.attribute.index', compact('datas', 'sl'));
    }

    public function store(Request $request){
        $validatedData = $request->validate([
            'name' => ['required', 'unique:attributes,name,'.$request->id.',id,service,car'],
        ...
        return back()->with('success', 'New Car Attribute Added Successfully!');
    }
    
    public function edit($id){
        $data = Attribute::findorFail($id);

        return view('admin.module.car.attribute.details', compact('data'));
    }
    
    public function update(Request $request, $id){
        $validatedData = $request->validate([
            'name' => ['required', 'unique:attributes,name,'.$id.',id,service,car'],
            'position' => ['nullable', 'numeric'],
        ]);
        $parentName = 'car';
        $data = Attribute::findorFail($id);
        ...
        return back()->with('success', 'Car Attribute Updated Successfully!');
    }
    ...
    public function termList($id){
        ...
        return view('admin.module.car.attribute.termList', compact('attributeInfo', 'datas', 'sl'));
    }
    ...
    public function termEdit($id){
        ...
        return view('admin.module.car.attribute.termDetails', compact('data'));
    }
